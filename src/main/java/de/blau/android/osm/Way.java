...
public class Way extends StyledOsmElement implements WayInterface, BoundedObject {
    ...
    public static boolean isSelfIntersecting(List<Node> nodes) {
        int n = nodes.size();
        for (int i = 0; i < n - 1; i++) {
            int[] p1 = getPoint(nodes.get(i));
            int[] p2 = getPoint(nodes.get(i + 1));

            for (int j = i + 2; j < n - 1; j++) {
                if (i == 0 && j == n - 2) continue; // пропускаем замыкающий сегмент

                int[] q1 = getPoint(nodes.get(j));
                int[] q2 = getPoint(nodes.get(j + 1));
    ...
    private static int[] getPoint(Node node) {
        if (node == null) return null;
        return new int[] {node.getLat(), node.getLon()};
    }

    private static boolean doIntersect(int[] p1, int[] p2, int[] q1, int[] q2) {
        if (Objects.isNull(p1) || Objects.isNull(p2) || Objects.isNull(q1) || Objects.isNull(q2)) return false;
    ...
    private static int orientation(int[] a, int[] b, int[] c) {
        long val = (long)(b[1] - a[1]) * (c[0] - b[0]) -
                (long)(b[0] - a[0]) * (c[1] - b[1]);
        if (val == 0) return 0;
        return (val > 0) ? 1 : 2;
    }

    private static boolean onSegment(int[] a, int[] b, int[] c) {
